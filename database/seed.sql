...
-- Види страхування (7 видів із різними ставками)
...
-- Агенти (12 агентів у різних філіях)
...
-- Клієнти (75 клієнтів з різними даними)
-- Перша група: 25 клієнтів
...
-- Продовження клієнтів (ще 50, щоб загалом було 75)
-- Використання PL/SQL-блоку для продуктивності
BEGIN
    FOR i IN 26..75 LOOP
...
-- Спеціальний клієнт: клієнт з усіма типами страхування (ClientID = 1)
-- Спеціальний клієнт: клієнт, який буде заблокований (ClientID = 2 — багато страхових випадків)

-- Користувачі (адмін, агенти, клієнти)
...
-- Договори (400+ договорів за різні періоди і статуси)
-- Контракти за січень 2024 (50 договорів)
BEGIN
    FOR i IN 1..50 LOOP
...
-- Контракти за лютий 2024 (60 договорів — для пікового місяця)
BEGIN
    FOR i IN 1..60 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Контракти за березень 2024 (55 договорів)
BEGIN
    FOR i IN 1..55 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Попередній місяць (листопад 2024) — для звітів по доходах агентів (40 договорів)
BEGIN
    FOR i IN 1..40 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Контракти за грудень 2024 (45 договорів)
BEGIN
    FOR i IN 1..45 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Додаткові контракти за 2023–2024 (150 договорів)
BEGIN
    FOR i IN 1..150 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Спеціальний кейс: клієнт 1 з усіма типами страхування (7 договорів)
BEGIN
    FOR i IN 1..7 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Спеціальний кейс: клієнт 2 з великою кількістю договорів (для перевірки блокування)
BEGIN
    FOR i IN 1..10 LOOP
        INSERT INTO Contract (ContractID, ClientID, AgentID, InsuranceTypeID, StartDate, EndDate, InsuranceAmount, Status)
        VALUES (
...
-- Страхові випадки (150 випадків із різним рівнем пошкоджень)
-- Звичайні страхові випадки (120)
BEGIN
    FOR i IN 1..120 LOOP
...
-- Спеціально: 6 випадків для клієнта 2 (більше ніж 50% від 10 договорів → має бути заблокований)
BEGIN
    FOR i IN 1..6 LOOP
        INSERT INTO InsuranceCase (CaseID, ContractID, CaseDate, ActNumber, DamageLevel, PaymentDate)
        VALUES (
...
-- Додаткові випадки за листопад 2024 (20 штук)
BEGIN
    FOR i IN 1..20 LOOP
        INSERT INTO InsuranceCase (CaseID, ContractID, CaseDate, ActNumber, DamageLevel, PaymentDate)
        VALUES (
...
-- Додаткові випадки (щоб досягти ~150)
BEGIN
    FOR i IN 1..10 LOOP
        INSERT INTO InsuranceCase (CaseID, ContractID, CaseDate, ActNumber, DamageLevel, PaymentDate)
        VALUES (
